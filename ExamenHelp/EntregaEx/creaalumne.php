...
if (!$connexio)
	...
	if(!mysqli_select_db($connexio,$nomBD)){
        ...
        $consulta = sprintf('select * from alumnes where nom="%s" ', $_POST["NOM"]);
        echo $consulta;
        ...
        if ($result = mysqli_query($connexio, $consulta)  )
        {
			...
			//Crear quan no hi hagi cap usuari amb el NOM introduit
                  ...
                  //echo $consulta;
			                                $consulta = sprintf('insert into alumnes (DNI, NOM, COGNOMS, ADREÇA, TELEFON, EDAT) values ("%s", "%s", "%s", "%s", "%s", "%s")', $_POST["DNI"], $_POST["NOM"], $_POST["COGNOMS"], $_POST["ADREÇA"], $_POST["TELEFON"], $_POST["EDAT"]);
